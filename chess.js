...
class Board {
    ...
    getWhiteMoves() {
        ...
        //Rooks
        let lightRookMoves = this.calcRookMoveBitBoard(
            this.lightRooks,
            this.lightPieces,
            this.darkPieces
        );
        ...
        //Bishops
        let lightBishopMoves = this.calcBishopMoveBitBoard(
            this.lightBishops,
            this.lightPieces,
            this.darkPieces
        );
        ...
        //Queen
        let lightQueenMoves = this.calcQueenMoveBitboard(
            this.lightQueen,
            this.lightPieces,
            this.darkPieces
        );
        ...
        //King
        let lightKingMoves = this.calcKingMoveBitBoard(
            this.lightKing,
            this.lightPieces,
            this.darkPieces,
            this.lightCastleRights,
            true
        );
    ...
    getBlackMoves() {
        ...
        let darkRookMoves = this.calcRookMoveBitBoard(
            this.darkRooks,
            this.darkPieces,
            this.lightPieces
        );
        let darkKnightMoves = this.calcKnightMoveBitBoard(this.darkKnights, this.darkPieces);
        let darkBishopMoves = this.calcBishopMoveBitBoard(
            this.darkBishops,
            this.darkPieces,
            this.lightPieces
        );
        let darkQueenMoves = this.calcQueenMoveBitboard(
            this.darkQueen,
            this.darkPieces,
            this.lightPieces
        );
        let darkKingMoves = this.calcKingMoveBitBoard(
            this.darkKing,
            this.darkPieces,
            this.lightPieces,
            this.darkCastleRights,
            false
        );
        return [
            darkPawnMoves,
    ...
    checkEnPassant(color, bitboard, pinningRays) {
        ...
        if (color) {
            if (this.lightKing & rowFive) {
                if (
                    countBits(rowFive & pinningRays & this.lightPieces) == 2 &&
                    countBits(rowFive & pinningRays & this.darkPieces) == 2
                )
                    return 0n;
            }
        ...
        if (this.darkKing & rowFour) {
            if (
                countBits(rowFour & pinningRays & this.darkPieces) == 2 &&
                countBits(rowFour & pinningRays & this.lightPieces) == 2
            )
                return 0n;
        }
    ...
    calcKnightMoveBitBoard(knightBitmap, currentPieces) {
        ...
        for (let i = 0; i < 64; i++) {
            if ((mask & knightBitmap) != 0n) {
                ...
                temp &= noHMask;
                temp >>= 9n;
                if ((temp & currentPieces) == 0n) knightMoves |= temp;
    ...
    calcKingMoveBitBoard(kingBitmap, currentPieces, otherPieces, castleRights, color) {
        ...
        if (color) {
            ...
            otherRookAttacks = this.calcRookMoveBitBoard(
                this.darkRooks,
                this.darkPieces,
                this.lightPieces & ~this.lightKing
            );
            otherKnightAttacks = this.calcKnightMoveBitBoard(this.darkKnights, otherPieces);
            otherBishopAttacks = this.calcBishopMoveBitBoard(
                this.darkBishops,
                this.darkPieces,
                this.lightPieces & ~this.lightKing
            );
            otherQueenAttacks = this.calcQueenMoveBitboard(
                this.darkQueen,
                this.darkPieces,
                this.lightPieces & ~this.lightKing
            );
            ...
            if ((kingBitmap & mask) != 0n) {
                ...
                if ((temp & currentPieces) == 0n)
                    if ((temp & otherAttacks) == 0n)
                        if (!this.isDefended(temp, color)) kingMoves |= temp;
                ...
                if (castleRights[1]) {
                    temp = mask;
                    if (
                        ((temp << 1n) & currentPieces) == 0n &&
                        ((temp << 2n) & currentPieces) == 0n &&
                        ((temp << 3n) & currentPieces) == 0n
                    )
                        if (
                            ((temp << 1n) & otherAttacks) == 0n &&
                            ((temp << 2n) & otherAttacks) == 0n &&
                            ((temp << 3n) & otherAttacks) == 0n
                        )
                            kingMoves |= temp << 2n;
                }
            }
    ...
    isDefended(sq, color) {
        ...
        let queenDefends;
        if (color) {
            rookDefends = this.calcRookMoveBitBoard(
                this.darkRooks,
                this.lightPieces,
                this.darkPieces
            );
            for (let i = 0; i < rookDefends.length; i++) if (rookDefends[i] & sq) return true;

            bishopDefends = this.calcBishopMoveBitBoard(
                this.darkBishops,
                this.lightPieces,
                this.darkPieces
            );
            for (let i = 0; i < bishopDefends.length; i++) if (bishopDefends[i] & sq) return true;

            knightDefends = this.calcKnightMoveBitBoard(this.darkKnights, this.lightPieces);
            for (let i = 0; i < knightDefends.length; i++) if (knightDefends[i] & sq) return true;

            queenDefends = this.calcQueenMoveBitboard(
                this.darkQueen,
                this.lightPieces,
                this.darkPieces
            );
            for (let i = 0; i < queenDefends.length; i++) if (queenDefends[i] & sq) return true;

            //Pawns (Manual Check)
            if (((sq & noAMask & no1Mask) << 9n) & this.darkPawns) return true;
            if (((sq & noHMask & no1Mask) << 7n) & this.darkPawns) return true;

            //King (Manual Check)
            if (((sq & noAMask & no1Mask) << 9n) & this.darkKing) return true;
            if (((sq & noHMask & no1Mask) << 7n) & this.darkKing) return true;
            if (((sq & no1Mask) << 8n) & this.darkKing) return true;
            if (((sq & noAMask) << 1n) & this.darkKing) return true;
            if (((sq & noHMask) >> 1n) & this.darkKing) return true;
            if (((sq & no8Mask) >> 8n) & this.darkKing) return true;
            if (((sq & no8Mask & noAMask) >> 7n) & this.darkKing) return true;
            if (((sq & no8Mask & noHMask) >> 9n) & this.darkKing) return true;
        } else {
            rookDefends = this.calcRookMoveBitBoard(
                this.lightRooks,
                this.darkPieces,
            ...
            //Pawns (Manual Check)
            if (((sq & noAMask & no1Mask) << 9n) & this.lightPawns) return true;
            if (((sq & noHMask & no1Mask) << 7n) & this.lightPawns) return true;

            //King (Manual Check)
            if (((sq & noAMask & no1Mask) << 9n) & this.lightKing) return true;
            if (((sq & noHMask & no1Mask) << 7n) & this.lightKing) return true;
            if (((sq & no1Mask) << 8n) & this.lightKing) return true;
            if (((sq & noAMask) << 1n) & this.lightKing) return true;
            if (((sq & noHMask) >> 1n) & this.lightKing) return true;
            if (((sq & no8Mask) >> 8n) & this.lightKing) return true;
            if (((sq & no8Mask & noAMask) >> 7n) & this.lightKing) return true;
            if (((sq & no8Mask & noHMask) >> 9n) & this.lightKing) return true;
        }
        return false;
    }
    ...
    checksAndPins(color, pinningRays, temp, move) {
        ...
        if (color) {
            ...
            if (this.darkInCheck[0]) {
                for (let j = 0; j < this.darkInCheck[1].length; j++) move &= this.darkInCheck[1][j];
            }
        }
        ...
        if (pinningRays & temp) {
            ...
            if (!color) {
                ...
                for (let i = 0; i < raysFromKing.length; i++) {
                    if (raysFromKing[i] & ~this.darkKing & pinningRays) {
                        // Checks if more than 1 piece in ray
                        if (
                            !(
                                raysFromKing[i] &
                                pinningRays &
    ...
    getLegalFromSquare(initialSquare) {
        ...
        for (let i = 1; i <= 8; i++) {
            for (let j = 65; j <= 72; j++) {
                if (String.fromCharCode(j) + i == initialSquare) sq = mask;
                mask >>= 1n;
            }
        }
    ...
    getPinningRays(color) {
        ...
        if (color) {
            ...
            for (let i = 0; i < 64; i++) {
                if (mask & this.darkRooks) rookMovesArr = rookMovesArr.concat(this.castRays(mask));
            ...
            for (let i = 0; i < rookMovesArr.length; i += 2) {
                for (let j = 0; j < raysFromKing.length; j++) {
                    if (
                        rookMovesArr[i] & this.lightKing &&
                        raysFromKing[j] & ~this.lightKing & rookMovesArr[i]
                    ) {
                        if (
                            rookMovesArr[i] &
                            raysFromKing[j] &
            ...
            for (let i = 1; i < bishopMovesArr.length; i += 2) {
                for (let j = 0; j < raysFromKing.length; j++) {
                    if (
                        bishopMovesArr[i] & this.lightKing &&
                        raysFromKing[j] & ~this.lightKing & bishopMovesArr[i]
                    ) {
                        if (
                            bishopMovesArr[i] &
                            raysFromKing[j] &
            ...
            for (let i = 0; i < queenMovesArr.length; i++) {
                for (let j = 0; j < raysFromKing.length; j++) {
                    if (
                        queenMovesArr[i] & this.lightKing &&
                        raysFromKing[j] & ~this.lightKing & queenMovesArr[i]
                    ) {
                        if (
                            queenMovesArr[i] &
                            raysFromKing[j] &
            ...
            for (let i = 0; i < 64; i++) {
                if (mask & this.lightRooks) rookMovesArr = rookMovesArr.concat(this.castRays(mask));
            ...
            for (let i = 0; i < rookMovesArr.length; i += 2) {
                for (let j = 0; j < raysFromKing.length; j++) {
                    if (
                        rookMovesArr[i] & this.darkKing &&
                        raysFromKing[j] & ~this.darkKing & rookMovesArr[i]
                    ) {
            ...
            for (let i = 1; i < bishopMovesArr.length; i += 2) {
                for (let j = 0; j < raysFromKing.length; j++) {
                    if (
                        bishopMovesArr[i] & this.darkKing &&
                        raysFromKing[j] & ~this.darkKing & bishopMovesArr[i]
                    ) {
                        if (
                            bishopMovesArr[i] &
                            raysFromKing[j] &
            ...
            for (let i = 0; i < queenMovesArr.length; i++) {
                for (let j = 0; j < raysFromKing.length; j++) {
                    if (
                        queenMovesArr[i] & this.darkKing &&
                        raysFromKing[j] & ~this.darkKing & queenMovesArr[i]
                    ) {
    ...
    getSingleVector(bitboardArr, king, i, j) {
        if (i == 0) {
            ...
            if (king == this.lightKing) {
                if ((king >> 9n) & noHMask & this.darkPawns) {
                    if (this.darkEnPassant)
                        return (
                            (king & bitboardArr[i][j]) | (king >> 9n) | (this.darkEnPassant >> 8n)
                        );
                ...
                if ((king >> 7n) & noAMask & this.darkPawns) {
                    if (this.darkEnPassant)
                        return (
                            (king & bitboardArr[i][j]) | (king >> 7n) | (this.darkEnPassant >> 8n)
                        );
                ...
                if ((king << 7n) & noHMask & this.lightPawns) {
                    if (this.lightEnPassant)
                        return (
                            (king & bitboardArr[i][j]) | (king << 7n) | (this.lightEnPassant << 8n)
                        );
                    return (king & bitboardArr[i][j]) | (king << 7n);
                }
        ...
        if (i == 1) {
            ...
            if (king == this.lightKing) {
                ...
                return (
                    (king & bitboardArr[i][j]) |
                    (this.getSumOfSinglePieceAttacks(rookAttacks) & bitboardArr[i][j]) |
                    (this.getSumOfSinglePieceAttacks(rookAttacks) & this.darkRooks)
                );
            } else {
                ...
                return (
                    (king & bitboardArr[i][j]) |
                    (this.getSumOfSinglePieceAttacks(rookAttacks) & bitboardArr[i][j]) |
                    (this.getSumOfSinglePieceAttacks(rookAttacks) & this.lightRooks)
                );
            }
        }
        if (i == 2) {
            ...
            let knightAttacks;
            if (king == this.lightKing) {
                knightAttacks = this.calcKnightMoveBitBoard(
                    king,
                    this.lightPieces,
                    this.darkPieces
                );
                return (
                    (king & bitboardArr[i][j]) |
                    (this.getSumOfSinglePieceAttacks(knightAttacks) & this.darkKnights)
                );
            } else {
                knightAttacks = this.calcKnightMoveBitBoard(
                    king,
                ...
                return (
                    (king & bitboardArr[i][j]) |
                    (this.getSumOfSinglePieceAttacks(knightAttacks) & this.lightKnights)
                );
            }
        }
        if (i == 3) {
            ...
            let bishopAttacks;
            if (king == this.lightKing) {
                bishopAttacks = this.calcBishopMoveBitBoard(
                    king,
                    this.lightPieces,
                    this.darkPieces
                );
                return (
                    (king & bitboardArr[i][j]) |
                    (this.getSumOfSinglePieceAttacks(bishopAttacks) & bitboardArr[i][j]) |
                    (this.getSumOfSinglePieceAttacks(bishopAttacks) & this.darkBishops)
                );
            } else {
                bishopAttacks = this.calcBishopMoveBitBoard(
                    king,
                ...
                return (
                    (king & bitboardArr[i][j]) |
                    (this.getSumOfSinglePieceAttacks(bishopAttacks) & bitboardArr[i][j]) |
                    (this.getSumOfSinglePieceAttacks(bishopAttacks) & this.lightBishops)
                );
            }
        }
        if (i == 4) {
            ...
            for (let i = 0; i < blackMoves.length; i++)
                for (let j = 0; j < blackMoves[i].length; j++)
                    if (blackMoves[i][j] & this.lightKing) {
                        ...
                        this.lightInCheck[1].push(
                            this.getSingleVector(blackMoves, this.lightKing, i, j)
                        );
                    }
            ...
            for (let i = 0; i < whiteMoves.length; i++)
                for (let j = 0; j < whiteMoves[i].length; j++)
                    if (whiteMoves[i][j] & this.darkKing) {
                        ...
                        this.darkInCheck[1].push(
                            this.getSingleVector(whiteMoves, this.darkKing, i, j)
                        );
                    }
            blackMoves = this.getBlackMoves();
        }
    ...
    getAllLegal() {
        let [whiteMoves, blackMoves] = this.calcChecks(this.playerBoolean);
        if (this.playerBoolean) return whiteMoves;
        return blackMoves;
    }
    ...
    getLegal(sq) {
        ...
        //Calc Checks
        let color;
        if (sq & this.lightPieces) color = true;
        else color = false;
        let [whiteMoves, blackMoves] = this.calcChecks(color);
        ...
        if ((sq & this.lightPieces) != 0n) {
            ...
            if (sq & this.lightKing) return whiteMoves[5];
        }

        if (sq & this.darkPieces) {
            //Dark piece found, return dark moves
            if (sq & this.darkPawns) return blackMoves[0][this.getMoveArrIndex(sq, this.darkPawns)];
            if (sq & this.darkRooks) return blackMoves[1][this.getMoveArrIndex(sq, this.darkRooks)];
            ...
            if (sq & this.darkQueen) return blackMoves[4][this.getMoveArrIndex(sq, this.darkQueen)];
            if (sq & this.darkKing) return blackMoves[5];
        }
    ...
    bitboardToArray(bitboard) {
        ...
        for (let i = 1; i <= 8; i++) {
            for (let j = 65; j <= 72; j++) {
                if (mask & bitboard) squareArray.push(String.fromCharCode(j) + i);
                mask >>= 1n;
            }
        }
    ...
    getSumOfSinglePieceAttacks(bitmapArr) {
        let attacks = 0n;
        for (let i = 0; i < bitmapArr.length; i++) attacks |= bitmapArr[i];
    ...
    doPromotion(sq, piece, color) {
        if (color) {
            ...
            if (piece == 2) this.lightRooks |= sq;
            if (piece == 3) this.lightKnights |= sq;
            if (piece == 4) this.lightBishops |= sq;
        } else {
            this.darkPawns &= ~sq;
            if (piece == 1) this.darkQueen |= sq;
            if (piece == 2) this.darkRooks |= sq;
            if (piece == 3) this.darkKnights |= sq;
            if (piece == 4) this.darkBishops |= sq;
        }
    }
    ...
    makeMoveFromText(start, target) {
        ...
        for (let i = 1; i <= 8; i++) {
            for (let j = 65; j <= 72; j++) {
                if (String.fromCharCode(j) + i == start) startBin |= mask;
                if (String.fromCharCode(j) + i == target) targetBin |= mask;
                mask >>= 1n;
            }
        }
    ...
    makeMove(start, target) {
        ...
        if (target & this.lightRooks) {
            if (target & h1) this.lightCastleRights[0] = false;
            if (target & a1) this.lightCastleRights[1] = false;
        }

        //Check dark castling rights
        if (target & this.darkRooks) {
            if (target & h8) this.lightCastleRights[0] = false;
            if (target & a8) this.lightCastleRights[1] = false;
        }
        ...
        if (start & this.lightPawns) {
            ...
            if (start & rowFive) {
                if (((start & noAMask) << 1n) & this.darkPawns) {
                    if (target & (start >> 7n)) {
                        if (!(target & this.darkPieces)) this.darkPawns &= ~(start << 1n);
                    }
                }
        ...
        if (start & this.darkPawns) {
            ...
            if (start & rowFour) {
                if (((start & noAMask) << 1n) & this.lightPawns) {
                    if (target & (start << 9n)) {
                        if (!(target & this.lightPieces)) this.lightPawns &= ~(start << 1n);
                    }
                }
    ...
    undoMove() {
        ...
        if (this.lightKing & this.lastMove[1]) {
            this.lightKing &= ~this.lastMove[1];
            this.lightKing |= this.lastMove[0];
        }
        if (this.lightRooks & this.lastMove[1]) {
            this.lightRooks &= ~this.lastMove[1];
            this.lightRooks |= this.lastMove[0];
        }
        if (this.lightBishops & this.lastMove[1]) {
            this.lightBishops &= ~this.lastMove[1];
            this.lightBishops |= this.lastMove[0];
        }
        if (this.lightKnights & this.lastMove[1]) {
            this.lightKnights &= ~this.lastMove[1];
            this.lightKnights |= this.lastMove[0];
        }
        if (this.lightQueen & this.lastMove[1]) {
            this.lightQueen &= ~this.lastMove[1];
            this.lightQueen |= this.lastMove[0];
        }
        if (this.lightPawns & this.lastMove[1]) {
            this.lightPawns &= ~this.lastMove[1];
            this.lightPawns |= this.lastMove[0];
        }

        if (this.darkKing & this.lastMove[1]) {
            this.darkKing &= ~this.lastMove[1];
            this.darkKing |= this.lastMove[0];
        }
        if (this.darkRooks & this.lastMove[1]) {
            this.darkRooks &= ~this.lastMove[1];
            this.darkRooks |= this.lastMove[0];
        }
        if (this.darkBishops & this.lastMove[1]) {
            this.darkBishops &= ~this.lastMove[1];
            this.darkBishops |= this.lastMove[0];
        }
        if (this.darkKnights & this.lastMove[1]) {
            this.darkKnights &= ~this.lastMove[1];
            this.darkKnights |= this.lastMove[0];
        }
        if (this.darkQueen & this.lastMove[1]) {
            this.darkQueen &= ~this.lastMove[1];
            this.darkQueen |= this.lastMove[0];
        }
        if (this.darkPawns & this.lastMove[1]) {
            this.darkPawns &= ~this.lastMove[1];
            this.darkPawns |= this.lastMove[0];
        }
    ...
    getActivePieces() {
        if (this.playerBoolean) return this.lightPieces;
